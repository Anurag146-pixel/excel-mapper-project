...
def extract_key_values(grid, start_r, end_r):
    """
    NON-TABULAR EXTRACTION - DO NOT MODIFY
    Extract key-value pairs from rows between start_r and end_r
    """
    data = {}
    last_key = None
...
def extract_table_with_template_bounds(grid, sec):
    ...
    # Step 6: Find header row
    header_row = None
    header_row_idx = None
    
    for r in range(header_start_r, min(end_r, len(grid))):
        full_row = grid[r]
        ...
        if is_header_row(clean_row):
            ...
            header_row_idx = r
            print(f"     Header row found at row {r}: {header_row}")
            break
    ...
    if header_row is None:
        for r in range(header_start_r, min(end_r, len(grid))):
            ...
            if clean_row:
                header_row = clean_row
                header_row_idx = r
                print(f"     No clear header, using row {r}: {header_row}")
                break
    
    if header_row is None:
        print(f"     ERROR: No header row found in range {header_start_r} to {end_r}")
        return {"records": [], "count": 0, "headers": []}
    ...
    for r in range(header_row_idx + 1, end_r):
    ...
    print(f"     ✓ Column range: {start_c} to {end_c}")
