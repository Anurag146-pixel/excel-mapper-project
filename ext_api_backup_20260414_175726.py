...
def db_update_status(token_id: str, status: str, source: str = None, result: str = None):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Try with updated_at, fallback without
            try:
                ...
                if "updated_at" in update_parts[-1]:
                    update_parts = update_parts[:-1]
                query = f"UPDATE token_details SET {', '.join(update_parts)} WHERE token_id = %s"
                params.append(numeric_id)
                cur.execute(query, params)
                rows_affected = cur.rowcount
        ...
        conn.commit()
        logger.info(f"DB  │ token={token_id}  numeric_id={numeric_id}  status={status}  source={source}  result={result}  rows_affected={rows_affected}")
