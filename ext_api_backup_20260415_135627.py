...
def db_update_status(token_id: str, status: str, source: str = None, result: str = None):
    ...
    Handles both formatted (token_N) and numeric (N) token_id formats.
    CRITICAL: Only appends token_id ONCE at the end to avoid parameter mismatches.
    """
    rows_affected = 0
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Build dynamic update query - ADD FIELDS FIRST, token_id LAST
            ...
            if source:
                ...
                logger.info(f"    Setting source_file_name = {source}")
            if result:
                update_parts.append("result_file_name = %s")
                params.append(result)
                logger.info(f"    Setting result_file_name = {result}")
            
            # TRY with updated_at, but fallback to query without it
            query_with_timestamp = f"UPDATE token_details SET {', '.join(update_parts)}, updated_at = NOW() WHERE token_id = %s"
            query_without_timestamp = f"UPDATE token_details SET {', '.join(update_parts)} WHERE token_id = %s"
            
            # Append numeric_id ONCE at the very end
            params.append(numeric_id)
            
            try:
                cur.execute(query_with_timestamp, params)
                rows_affected = cur.rowcount
            except Exception as timestamp_err:
                # Try without updated_at field
                logger.debug(f"Query with timestamp failed: {timestamp_err}. Trying without timestamp...")
                cur.execute(query_without_timestamp, params)
                rows_affected = cur.rowcount
            
        conn.commit()
        logger.info(f"✅ DB UPDATE: token={token_id}  numeric_id={numeric_id}  status={status}  rows_affected={rows_affected}")
        
        if rows_affected == 0:
            logger.warning(f"⚠️  WARNING: Update returned 0 rows affected for token {token_id}")
...
def db_create_token():
    """
    Create a new token and return the token_id (formatted as token_N).
    Database stores numeric ID, but API returns formatted string.
    """
    try:
        conn = pymysql.connect(**DB_CONFIG)
        with conn.cursor() as cur:
            try:
                cur.execute(
                    """
                    INSERT INTO token_details (status, source_file_name, template_name, result_file_name, created_at, updated_at)
                ...
                try:
                    cur.execute(
                        """
                        INSERT INTO token_details (status, source_file_name, template_name, result_file_name)
                        VALUES ('pending', '', '', '')
                        """
                    )
                except Exception as fallback_err:
                    logger.error(f"Token creation failed: {str(fallback_err)}")
                    logger.error(f"Fallback error details: {fallback_err}")
                    traceback.print_exc()
                    raise
                    
            conn.commit()
            numeric_id = cur.lastrowid
            token_id_formatted = f"token_{numeric_id}"
            logger.info(f"Token created: numeric_id={numeric_id}  formatted={token_id_formatted}")
...
def run_extraction_pipeline(token_id: str, source_file_name: str, template_name: str, protocol: str = "REST"):
    ...
    try:
        ...
        # ── 8. Mark DB done ───────────────────────────────────────────────────────
        logger.info(f"[{protocol}] Step 8: Updating database with 'done' status...")
